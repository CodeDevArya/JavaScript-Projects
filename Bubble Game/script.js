function main() {
    ...
    function getScore() {
        score += 1;
        if (score > Hhiscore_value) {
            Hhiscore_value = score;
            localStorage.setItem("Hhiscore", JSON.stringify(Hhiscore_value))
            document.querySelector("#hiboxscore").innerHTML = Hhiscore_value;
        }
...
let Hhiscore = localStorage.getItem("Hhiscore");
if (Hhiscore === undefined || Hhiscore === null) {
    Hhiscore_value = 0;
    localStorage.setItem("Hhiscore", JSON.stringify(Hhiscore_value));
}
else {
    Hhiscore_value = JSON.parse(Hhiscore);
    document.querySelector("#hiboxscore").innerHTML = Hhiscore;
}
